...
def get_response_time(DG, source, sink, overhead):
    ...
    for node in DG.nodes:
        ...
        sourcetobottle = (len(shortestsourcetonodepath)) * waitingtimes[node] #-1 IS MODIFICATION TO TRY TO BRING EQUAL TO NEW ANALYSIS
...
def main(filename):
    DG = nx.DiGraph(nx.nx_pydot.read_dot(filename))
    ...
    return get_response_time(DG, source[0], sink[0], False)
...
if __name__ == "__main__":
    ...
    print(f"Elapsed time: {end1 - start:.10f} seconds")
